					...
					uint64_t blk_num)
{
	...
	if (segno != this->buffer.segno)
		this->jops->load(this, blk_num * RECORDS_PER_BLOCK,
				 this->record_end);
			   ...
			   struct journal_block *buffer)
{
	...
	rc = journal_derive_key(this->superblock->root_key, blk_num, blk_key,
				AES_GCM_KEY_SIZE);
	if (rc) {
	...
	rc = this->cipher->encrypt(this->cipher, (char *)cur_blk,
				   BLOCK_CRYPT_LEN, blk_key, blk_iv, blk_mac,
				   blk_num, (char *)buffer);
	if (rc)
...
void journal_decrypt_block(struct journal_region *this, uint64_t blk_num)
{
	...
	rc = this->cipher->decrypt(this->cipher, (char *)cur_blk,
				   BLOCK_CRYPT_LEN, blk_key, blk_iv, blk_mac,
				   blk_num, (char *)cur_blk);
	if (rc)
			 ...
			 uint64_t record_end)
{
	...
	if (record_end > record_start &&
	    segno == (record_end / RECORDS_PER_SEGMENT)) {
	...
	for (i = 0; i < blk_count; i++) {
		blk_pba =
			this->superblock->journal_region_start + blk_start + i;
...
void journal_synchronize(struct journal_region *this)
{
	...
	if (this->record_end % RECORDS_PER_SEGMENT != 0) {
		...
		blk_count =
			BLOCKS_PER_SEGMENT - (blk_start % BLOCKS_PER_SEGMENT);
	}
...
uint64_t journal_add_record(struct journal_region *this,
			    struct journal_record *record)
{
	uint64_t ret;
	struct journal_block *blk;
...
uint64_t nop_add_record(struct journal_region *this,
			struct journal_record *record)
{
	return 0;
}
struct journal_block *nop_get_block(struct journal_region *this,
				    uint64_t blk_num)
{
	return NULL;
}
...
int journal_region_init(struct journal_region *this,
			struct superblock *superblock)
{
	...
	this->buffer.blocks = kzalloc(
		BLOCKS_PER_SEGMENT * sizeof(struct journal_block), GFP_KERNEL);
