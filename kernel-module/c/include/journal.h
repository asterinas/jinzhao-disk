...
#ifndef DM_JINDISK_JOURNAL_H
#define DM_JINDISK_JOURNAL_H
...
#define RECORDS_PER_BLOCK                                                      \
	((JOURNAL_BLOCK_SIZE - 2 * sizeof(struct crypto_info)) /               \
...
#define BLOCK_CRYPT_LEN                                                        \
	(sizeof(struct crypto_info) +                                          \
	 RECORDS_PER_BLOCK * sizeof(struct journal_record))
#define MAX_BITS 64
...
struct journal_operations {
	...
	uint64_t (*add_record)(struct journal_region *this,
